...
void specialGaussianElimination(vector<vector<unsigned int>>& batchEliminationRows, vector<vector<unsigned int>>& eliminationSubs) {
    ...
    for (int i = 0; i < row_tmp.size(); ++i) {
        // 遍历行中的每个元素，使用AVX加速
        for (int j = 0; j < row_tmp[i].size(); j += 8) {
            // 加载当前行和对应消元子到AVX寄存器
            __m256i rowVec = _mm256_loadu_si256((__m256i*)&row_tmp[i][j]);
            __m256i eleVec = _mm256_loadu_si256((__m256i*)&ele_tmp[j / 32][j]);

            // 计算AVX寄存器的按位与
            __m256i andResult = _mm256_and_si256(rowVec, eleVec);

            // 将结果存回内存
            _mm256_storeu_si256((__m256i*)&row_tmp[i][j], andResult);
        }
    }
